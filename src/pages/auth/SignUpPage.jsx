...
const SignUpPage = () => {
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      const { migrated } = await signup('google', { token });
      if (migrated) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      const { migrated } = await signup('phone', { token });
      if (migrated) {
        window.dispatchEvent(new CustomEvent('show-toast', {
          detail: { message: 'Your progress has been saved to your account!', type: 'success' }
        }));
  ...
  const handleEmailSignup = async (e) => {
    ...
    try {
      ...
      const { migrated } = await signup('email', { token });
      if (migrated) {
        window.dispatchEvent(new CustomEvent('show-toast', {
          detail: { message: 'Your progress has been saved to your account!', type: 'success' }
        }));
        ...
        <div className="space-y-3.5">
              ...
              <div className="p-5 space-y-4 animate-fade-in">
                {!otpSent ? (
                  <form onSubmit={handleSendOtp} className="space-y-3.5" noValidate>
                    <div>
                      <label htmlFor="signup-phone" className="sr-only">Mobile Number</label>
          ...
          <div className="border border-gray-200 rounded-xl bg-white overflow-hidden shadow-sm transition-all duration-300">
            ...
            {expandedOption === 'email' && (
              <form onSubmit={handleEmailSignup} className="p-5 space-y-4 animate-fade-in" noValidate>
                <div className="space-y-3.5">
                  <div>
